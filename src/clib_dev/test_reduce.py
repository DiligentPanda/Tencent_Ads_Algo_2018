...
from clib_dev.functions.tools import reduce,replicate
import torch

from torch.autograd import gradcheck

input = Variable(torch.Tensor([[1, 2], [4, 3], [5, 6]])).cuda()
offsets = Variable(torch.LongTensor([0, 2])).cuda()
results = reduce(input, offsets,"sum")
print(results)
...
input = (
    Variable(torch.Tensor([[1, 2], [4, 3], [5, 6]]).cuda(), requires_grad=True),
...
# output = reduce(input[0],input[1])
# output.backward(torch.Tensor([[1,0],[0,1]]).cuda())
# print(input[0].grad)
